...
| `save_genome_name` | If an alignment index is generated by the pipeline use this parameter to save it to the outdir with this parameter as the folder name. These can then be used for future pipeline runs, reducing processing times. | `string` |  |  |  |
